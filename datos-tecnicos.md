# Datos Técnicos — BITHORCOIN

| Dato | Valor |
|---|---|
| **Nombre** | BITHORCOIN |
| **Símbolo** | BTHOR |
| **Suministro total** | 1.000.000 BTHOR |
| **Red** | Polygon (MATIC) |
| **Estándar** | ERC-20 / RC-20 compatible |
| **Par de lanzamiento** | USDC |
| **Fecha de lanzamiento** | Octubre 2026 |
| **Preventa** | Ninguna |
| **Reservas del equipo** | Ninguna |
| **Dirección del contrato** 0xfe85068BF293e24b6BCdFBAfDbEB069C9B62AdF4
| **Enlace del pool** | Próximamente |

## Principios técnicos
- ✅ Sin acuñación posterior: no se creará ni una moneda más
- ✅ Transparencia: todo verificado en explorador de Polygon
- ✅ Comisiones bajas: ideal para uso diario
- ✅ Accesible desde cualquier billetera compatible

---
⚠️ No es consejo financiero. Investigá siempre por tu cuenta.
